...
export class MiddlewarePipeline {
  ...
  public async execute<T = any>(
    ...
    if (this._options.enableDebugLogging) {
      this._options.logger.debug(
        `[Pipeline] Starting execution for ${request.method} ${request.path}`
      );
    }
    ...
    try {
      ...
      const executeNext = async (): Promise<APIResponse<T>> => {
        ...
        if (chainIndex >= middlewareToExecute.length) {
          ...
          if (this._options.enableDebugLogging) {
            this._options.logger.debug(
              `[Pipeline] All middleware executed, handling request`
            );
          }
        ...
        if (this._options.enableDebugLogging) {
          this._options.logger.debug(
            `[Pipeline] Executing middleware: ${middlewareName}`
          );
        }
        ...
        try {
          ...
          if (context.shortCircuited) {
            ...
            if (this._options.enableDebugLogging) {
              this._options.logger.debug(
                `[Pipeline] Pipeline short-circuited by ${middlewareName}`
              );
            }
          }
          ...
          if (this._options.enableDebugLogging) {
            this._options.logger.debug(
              `[Pipeline] Middleware ${middlewareName} failed:`,
              error
            );
          }
            ...
            for (const errorMW of errorMiddleware) {
              if (errorMW.metadata.enabled) {
                try {
                  ...
                  if (this._options.enableDebugLogging) {
                    this._options.logger.debug(
                      `[Pipeline] Error recovered by ${errorMW.metadata.name}`
                    );
                  }
                  return result as APIResponse<T>;
                } catch (recoveryError) {
                  ...
                  if (this._options.enableDebugLogging) {
                    this._options.logger.debug(
                      `[Pipeline] Error recovery failed for ${errorMW.metadata.name}:`,
                      recoveryError
                    );
                  }
                }
    ...
    for (const middleware of errorMiddleware) {
      ...
      if (errorMW.handleError) {
        try {
          ...
          if (this._options.enableDebugLogging) {
            this._options.logger.debug(
              `[Pipeline] Error recovery failed for ${middleware.metadata.name}:`,
              recoveryError
            );
          }
        }
  ...
  private _updateMetrics(
    ...
    this._metrics.averageExecutionTime =
      this._executionTimeHistory.reduce((a, b) => a + b, 0) /
      this._executionTimeHistory.length;
    ...
    this._metrics.averageMiddlewareCount =
      (this._metrics.averageMiddlewareCount * (this._metrics.totalRequests - 1) +
        middlewareCount) /
      this._metrics.totalRequests;
  }
