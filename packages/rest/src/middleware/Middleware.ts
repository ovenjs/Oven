 ...
 */
export type MiddlewareFactory<T extends IMiddleware = IMiddleware> = (
  ...args: any[]
) => T;
...
export abstract class ResponseMiddleware
  extends BaseMiddleware
  implements IResponseMiddleware
{
  constructor(metadata: Omit<MiddlewareMetadata, 'type'>) {
    super({
      name: metadata.name,
  ...
  public abstract processResponse(
    context: MiddlewareContext & { response: APIResponse },
    next: () => Promise<APIResponse>
  ): Promise<APIResponse>;
}
...
export class MiddlewareRegistry {
  ...
  public getByType(type: MiddlewareType): IMiddleware[] {
    return Array.from(this._middleware.values())
      .filter(
        middleware => middleware.metadata.type === type && middleware.metadata.enabled
      )
