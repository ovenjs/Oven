...
export class EventEmitter {
  ...
  constructor(performanceMonitor?: PerformanceMonitor) {
    // Create a minimal logger if needed
    const logger = performanceMonitor ? new Logger() : new Logger();
    this.performanceMonitor = performanceMonitor || new PerformanceMonitor({}, logger);
  }
  ...
  on(
    ...
    if (listeners.length >= (eventOptions.maxListeners || this.defaultMaxListeners)) {
      if (eventOptions.throwOnMaxListeners) {
        throw new Error(
          `Max listeners (${eventOptions.maxListeners || this.defaultMaxListeners}) exceeded for event: ${event}`
        );
  ...
  async emit(event: string, ...args: any[]): Promise<boolean> {
    ...
    for (const metadata of listeners) {
      ...
      // Execute listener
      const listenerStartTime = this.now();
      try {
        ...
        const executionTime = this.now() - listenerStartTime;
