...
export class EventManager extends EventEmitter {
	 ...
	 * @param data Event data
	 */
	async emitResponseEvent(type: RestEventType, data: ResponseEventData): Promise<boolean> {
		return this.emitEvent(type, data);
	}
	 ...
	 * @param data Event data
	 */
	async emitPerformanceEvent(type: RestEventType, data: PerformanceEventData): Promise<boolean> {
		return this.emitEvent(type, data);
	}
	 ...
	 */
	getEventHistory(type?: string, limit = 100): Array<{
		type: string;
		data: any;
		timestamp: number;
	}> {
	...
	private persistEvents(): void {
		...
		try {
			...
			if (typeof globalThis !== 'undefined' && typeof globalThis.localStorage !== 'undefined') {
				globalThis.localStorage.setItem(this.config.eventPersistenceKey!, JSON.stringify(data));
			}
		} catch (error) {
