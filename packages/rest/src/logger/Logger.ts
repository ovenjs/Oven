...
export class Logger extends EventEmitter {
	 ...
	 * Log a trace message
	 */
	trace(message: string, meta?: Record<string, any>, requestId?: string, context?: string): void {
	 ...
	 * Log a debug message
	 */
	debug(message: string, meta?: Record<string, any>, requestId?: string, context?: string): void {
	 ...
	 * Log an info message
	 */
	info(message: string, meta?: Record<string, any>, requestId?: string, context?: string): void {
	 ...
	 */
	warn(message: string, meta?: Record<string, any>, requestId?: string, context?: string): void {
	 ...
	 */
	error(message: string, meta?: Record<string, any>, requestId?: string, context?: string): void {
	...
	private log(
		...
		if (this.config.enablePerformanceTracking && this.performanceMonitor) {
			this.performanceMonitor.incrementCounter('logging.entries', 1, { level: LogLevel[level] });
		}
	...
	startDebugSession(config: Partial<DebugSessionConfig> = {}): string {
		...
		this.addLogFilter((entry) => {
			...
			if (entry.meta?.type === 'network' && !sessionConfig.includeNetworkLogs) return false;
