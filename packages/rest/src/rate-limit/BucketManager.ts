...
import { Bucket, BucketOptions, QueuedRequest, BucketMetrics } from './Bucket';
...
export class BucketManager extends EventEmitter {
	...
	public async addRequest<T = any>(
		...
		if (this._globalBucket.isRateLimited) {
			...
			await new Promise(resolve => setTimeout(resolve, this._globalBucket.timeToNextRequest));
		}
		...
		const result = await bucket.addRequest<T>(request, priority, maxRetries, retryDelay, isBurst);
	 ...
	 * @param majorParameters The major parameters for the bucket
	 */
	private _generateBucketId(route: string, majorParameters?: Record<string, Snowflake>): string {
	...
	private _cleanupBuckets(): void {
		...
		for (const [bucketId, bucket] of this._buckets.entries()) {
			...
			const lastActivity = bucket.metrics.queueSize > 0 || bucket.metrics.activeRequests > 0 ? now : this._lastCleanup;
