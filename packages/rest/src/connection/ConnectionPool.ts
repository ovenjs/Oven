...
export class ConnectionPool extends EventEmitter {
    ...
    if (newSize > oldSize) {
      ...
      for (let i = 0; i < connectionsToAdd; i++) {
        ...
        const urls = Array.from(
          new Set(Array.from(this.connections.values()).map(conn => conn.url))
        );
        if (urls.length > 0) {
  ...
  private updateStats(): void {
    ...
    this.stats.activeConnections = connections.filter(
      conn => conn.status === ConnectionStatus.ACTIVE
    ).length;
    this.stats.idleConnections = connections.filter(
      conn => conn.status === ConnectionStatus.IDLE
    ).length;
    this.stats.closedConnections = connections.filter(
      conn => conn.status === ConnectionStatus.CLOSED
    ).length;
    this.stats.errorConnections = connections.filter(
