    ...
    return new Promise((resolve, reject) => {
      ...
      const item: BatchItem<T, R> = {
        ...
        metadata: {
          ...
          priority:
            options.priority || this.config.defaultPriority || BatchItemPriority.NORMAL,
          createdAt: now,
          retryCount: 0,
  ...
  private async executeBatch(items: BatchItem<T, R>[]): Promise<R[]> {
    ...
    for (const item of items) {
      try {
        ...
        const queueTime =
          (item.metadata.processingStartedAt || 0) - item.metadata.createdAt;
        ...
        this.stats.averageProcessingTime =
          this.stats.totalProcessingTime / this.stats.completedItems;
        this.stats.totalQueueTime += queueTime;
        this.stats.averageQueueTime =
          this.stats.totalQueueTime / this.stats.completedItems;
        this.updateStats();
    ...
    this.stats.averageBatchSize =
      (this.stats.averageBatchSize * (this.stats.totalBatches - 1) + items.length) /
      this.stats.totalBatches;
