...
export class RequestHandler {
    ...
    async batchRequests(requests) {
        ...
        for (const group of groups) {
            const groupResults = await Promise.allSettled(group.map(async (req) => {
                try {
            ...
            results.push(...groupResults.map(result => result.status === 'fulfilled' ? result.value : {
                success: false,
                error: result.reason,
                status: 0,
                headers: {},
    ...
    async performRequest(url, options) {
        ...
        const response = await request(finalUrl, {
            ...
            headers,
            body: body, // Cast to any for undici compatibility
