...
import type { Brand, Phantom } from '@ovenjs/types';
...
export interface HookManager {
  ...
  on<K extends keyof HookEvents>(
    event: K,
    listener: (...args: HookEvents[K]) => void
  ): void;
...
/**
 * Hook transformation interface
 */
export interface HookTransformation<TInput, TOutput> {
  readonly name: string;
...
/**
 * Hook filter interface
 */
export interface HookFilter<T> {
  readonly name: string;
  readonly filter: (value: T, context: HookExecutionContext) => boolean | Promise<boolean>;
}

/**
 * Hook aggregator interface
 */
export interface HookAggregator<TInput, TOutput> {
  readonly name: string;
  readonly aggregate: (values: TInput[]) => TOutput | Promise<TOutput>;
}
